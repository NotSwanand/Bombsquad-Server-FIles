...
class SoccerGame(bs.TeamGameActivity):
    ...
    def __init__(self,settings):
        ...
        self._puckModel = bs.getModel("impactBomb")
        self._puckTex = bs.getTexture("impactBombColorLit")
    ...
    def onTransitionIn(self):
    	...
    	self._modCreditSoccer = bs.NodeActor(bs.newNode('text',
													  attrs={'vAttach':'top',
															 'hAlign':'center',
															 'vrDepth':-30,
															 'color':(0,0,0, 0.5),
															 'shadow':2 if True else 0.5,
															 'flatness':1.0 if True else 0.5,
															 'position':(-360,-30),
															 'maxWidth':0,
															 'scale': 0.7 if True else 0.6,
															 'text':'Current Mod Credits\nKSR, FiYONM, YRT'}))
    ...
    def _handleScore(self):
        ...
        bs.pushCall(bs.Call(bs.screenMessage,'Goal!',color=(0,1,0),log=True))
