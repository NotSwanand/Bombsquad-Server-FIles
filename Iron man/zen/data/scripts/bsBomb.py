...
class BombFactory(object):
    ...
    def __init__(self):
        ...
        self.elonMineModel = bs.getModel('bonesHead')
        ...
        self.tntTex = bs.getTexture('achievementFlawlessVictory')
        self.forceTex = bs.getTexture('egg4')
        self.elonMineTex = bs.getTexture('powerupCurse')
        self.elonMineLitTex = bs.getTexture('achievementCrossHair')
    ...
    def handleMessage(self, msg):
        ...
        if isinstance(msg, bs.DieMessage):
            ...
            if node is not None and node.exists():
                ...
                elif self.blastType == 'elonMine': mag *= 2.0
...
class Bomb(bs.Actor):
    ...
    def __init__(self, position=(0,1,0), velocity=(0,0,0), bombType='normal',
        ...
        if not bombType in ('ice','impact','forceBomb','airstrike','landMine','normal','banana','sticky','fireBottle','tnt','elonMine'):
        ...
        if self.bombType == 'landMine':
            ...
            if self.bombType == 'landMine':
                self.bomb = bs.newNode('shield',owner=self.node,attrs={'color':(1,0,1),'radius':0.7})
                self.node.connectAttr('position',self.bomb,'position')
            ...
            self._trailTimer = bs.Timer(10,
                bs.WeakCall(self._addTrail), repeat=True)                
            if self.bombType == 'sticky':
                self.bomb = bs.newNode('shield',owner=self.node,attrs={'color':(1,0,1),'radius':2.9})
                self.node.connectAttr('position',self.bomb,'position')
            self._trailTimer = bs.Timer(10,
                bs.WeakCall(self._addTrail), repeat=True)                
        ...
        # light the fuse!!!
        if self.bombType not in ('landMine','tnt','airstrike','forceBomb','toxic','elonMine','heal'):
            bs.gameTimer(fuseTime,
                         bs.WeakCall(self.handleMessage, ExplodeMessage()))
            if self.bombType == 'normal':
                self.bomb = bs.newNode('shield',owner=self.node,attrs={'color':(1,0,1),'radius':0.9})
                self.node.connectAttr('position',self.bomb,'position')
            if self.bombType == 'ice':
                self.bomb = bs.newNode('shield',owner=self.node,attrs={'color':(1,0,1),'radius':1.2})
                self.node.connectAttr('position',self.bomb,'position')
    ...
    def arm(self):
        ...
        if not self.bombType in ['forceBomb']:
