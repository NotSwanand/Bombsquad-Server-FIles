...
class Map(bs.Actor):
    ...
    def __init__(self, vrOverlayCenterOffset=None):
        ...
        def snowfall():
                ...
                bs.emitBGDynamics(position=p,velocity=v,count=10,scale=4+random.random(),spread=0,chunkType='spark')
        
        bs.gameTimer(20,bs.Call(snowfall),repeat = True)        
        
        self._light = bs.Timer(1000,bs.WeakCall(self.light),repeat = True)
    ...
    def light(self):
                light = bs.newNode('light',
                                attrs={'position':(0,10,0),
                                    ...
                                    'radius':10})
                bsUtils.animate(light,"intensity",{0:1,70:10,250:15,250:20,360:30,410:50,510:0})
...
class ThePadMap(Map):
    ...
    def onPreload(cls):
        ...
        data['bgTex'] = bs.getTexture('shrapnel1Color')
...
class RampageMap(Map):
    ...
    def __init__(self):
        ...
        self.railing = bs.newNode('terrain', attrs={
            ...
            'materials':[bs.getSharedObject('railingMaterial')],
            'bumper':True})
        self._dynamicsEmitter = bs.Timer(50,bs.WeakCall(self.emitBGDynamics),repeat = True)
        bsGlobals = bs.getSharedObject('globals')
        bsGlobals.tint = (1.2, 1.1, 0.97)
        bsGlobals.ambientColor = (1.3, 1.2, 1.03)
        bsGlobals.vignetteOuter = (0.62, 0.64, 0.69)
        bsGlobals.vignetteInner = (0.97, 0.95, 0.93)
    def emitBGDynamics(self):
        bs.emitBGDynamics(position=(-15+random.random()*30,random.random()*1.2,-15+random.random()*30),count=int(5.0+random.random()*10),scale=3.0,spread=4.9,chunkType='spark')
