...
class SpazFactory(object):
    ...
    def __init__(self):
        ...
        self.railgunSound = bs.getSound('impactMedium')
        self.railgunChargeSound = bs.getSound('impactMedium2')
        ...
        if isinstance(msg, bs.PickedUpMessage):
            ...
            if (msg.powerupType == 'tripleBombs'):
                if bs.getConfig().get('Powerup Popups', True):
                    bsUtils.PopupText(('TRIPLE BOMBS'),
                        color=(0, 1, 0),
                        scale=1,
                ...
                if bs.getConfig().get('Powerup Popups', True):
                    bsUtils.PopupText(('JUMPING POWER!'),
                        color=(1, 1, 1),
                        scale=1,
                        position=self.node.position).autoRetain()
                ...
                if bs.getConfig().get('Powerup Popups', True):
                    bsUtils.PopupText(('LAND MINES!'),
                        color=(0, 1, 0),
                        scale=1,
                        position=self.node.position).autoRetain()
                self.setLandMineCount(min(self.landMineCount+3, 3))
            elif msg.powerupType == 'fly':
                if bs.getConfig().get('Powerup Popups', True):
                    bsUtils.PopupText(('I BELIEVE I CAN FLY!'),
                        color=(0, 1, 0),
                        scale=1,
                        position=self.node.position).autoRetain()                
                ...
                if bs.getConfig().get('Powerup Popups', True):
                    bsUtils.PopupText(('WHO WANTS TO EXPERIENCE ANTI-GRAVITY?'),
                        color=(1, 1, 1),
                        scale=1,
                        position=self.node.position).autoRetain()

                self.setForceBombCount(min(self.forceBombCount + 2,1))
            elif msg.powerupType == 'airstrike':
                if bs.getConfig().get('Powerup Popups', True):
                    bsUtils.PopupText(('DAMN\xf0\x9f\x94\xa5'),
                        color=(1, 1, 1),
                        scale=1,
                        position=self.node.position).autoRetain()

                self.setAirstrikeCount(self.airstrikeCount+1)
            elif msg.powerupType == 'railgun':
                if bs.getConfig().get('Powerup Popups', True):
                    bsUtils.PopupText(('KA..ME..HA..ME..HAA\xf0\x9f\x94\xa5'),
                        color=(0, 0.9, 1),
                        scale=1,
                        position=self.node.position).autoRetain()

                self.railgunCharges += 3
                bs.playSound(
                    self.getFactory().punchSound,
                ...
                bs.gameTimer(100, bs.Call(waitForCharge))
            elif msg.powerupType == 'elonMine':
                if bs.getConfig().get('Powerup Popups', True):
                    bsUtils.PopupText(('SEARCHING NOOBS!'),
                        color=(0, 1, 0),
                        scale=1,
                        position=self.node.position).autoRetain()
                ...
                if bs.getConfig().get('Powerup Popups', True):
                    bsUtils.PopupText(('WHERE THE FUCK DID HE GO?!?!\xf0\x9f\xa4\xaf\xf0\x9f\xa4\xab'),
                        color=(0, 1, 0),
                        scale=1,
                        position=self.node.position).autoRetain()            	
            ...
            elif msg.powerupType == 'cannon':
                if bs.getConfig().get('Powerup Popups', True):
                    bsUtils.PopupText(('BAZUKA!'),
                        color=(1, 1, 1),
                        scale=1,
                        position=self.node.position).autoRetain()
                ...
                if bs.getConfig().get('Powerup Popups', True):
                    bsUtils.PopupText(('RAINING DEATH WORKING! ANY FUCKER HERE?'),
                        color=(1, 0, 0),
                        scale=1,
                        position=self.node.position).autoRetain()
                ...
                if bs.getConfig().get('Powerup Popups', True):
                    bsUtils.PopupText(('DONT TRY BOMBJUMP WITH IT!'),
                        color=(1, 1, 1),
                        scale=1,
                        position=self.node.position).autoRetain()
                ...
                if bs.getConfig().get('Powerup Popups', True):
                    bsUtils.PopupText(('STICKY BOMBS'),
                        color=(0, 1, 0),
                        scale=1,
                        position=self.node.position).autoRetain()
                ...
                if bs.getConfig().get('Powerup Popups', True):
                    bsUtils.PopupText(('ITS COLD OUT THERE'),
                        color=(0, 0.9, 5),
                        scale=1,
                        position=self.node.position).autoRetain()
                self.bombType = 'ice'
            ...
            if msg.hitType == 'punch':
                ...
                if damage > 999:
                    ...
                    bsUtils.PopupText("PATT SE HEADSHOT!",color=(1,0,0),scale=1.6,position=self.node.position).autoRetain()
                elif damage >= 600 and damage < 700:
                    bsUtils.PopupText("SURPRISE MOTHERFUCKER!",color=(1,1,0),scale=1.6,position=self.node.position).autoRetain()
               
                elif damage >= 500 and damage < 600:
                    bsUtils.PopupText("KHANA NHI KHAYA KYA\xf0\x9f\x98\x92!",color=(0,1,0.3),scale=1.6,position=self.node.position).autoRetain()             
                elif damage >= 400 and damage < 500:
                    bsUtils.PopupText("OH! YEAH",color=(0,1,0.3),scale=1.6,position=self.node.position).autoRetain()
                elif damage >= 300 and damage < 400:
                    bsUtils.PopupText("APNE BAAP KO BULA!",color=(0.2,1,0.2),scale=1.6,position=self.node.position).autoRetain()
                elif damage >= 200 and damage < 300:
                    bsUtils.PopupText("WTF!",color=(0.1,0.1,0.5),scale=1.6,position=self.node.position).autoRetain()
                elif damage >= 100 and damage < 200:
                    bsUtils.PopupText(".....!",color=(0.7,0.9,0.7),scale=1.6,position=self.node.position).autoRetain()
                elif damage > 0 and damage < 100:
                    bsUtils.PopupText("!!!!!",color=(1,0.3,0.5),scale=1.6,position=self.node.position).autoRetain()
...
t.defaultColor = ((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)),
